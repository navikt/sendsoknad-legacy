...
public final class StofoTransformers {
    ...
    private static final Map<Class<?>, Function<String, ?>> TRANSFORMERS = new HashMap<>();
    private static final Map<Class<?>, Function<Faktum, ?>> FAKTUM_TRANSFORMERS = new HashMap<>();
    ...
    static {
        ...
        FAKTUM_TRANSFORMERS.put(Periode.class, StofoTransformers::faktumTilPeriode);
    ...
    public static <T> T extractValue(Faktum faktum, Class<T> clazz, String property) {
        ...
        try {
            if (FAKTUM_TRANSFORMERS.containsKey(clazz)) {
                result = FAKTUM_TRANSFORMERS.get(clazz).apply(faktum);
            } else if (StringUtils.isNotBlank(valueToConvert) && TRANSFORMERS.containsKey(clazz)) {
                result = TRANSFORMERS.get(clazz).apply(valueToConvert);
            } else {
                result = null;
            }
