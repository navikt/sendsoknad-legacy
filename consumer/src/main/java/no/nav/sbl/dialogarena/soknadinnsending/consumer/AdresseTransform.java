...
public class AdresseTransform {
    ...
    private Adresse finnMidlertidigAdresse(XMLMidlertidigPostadresseNorge xmlMidlPostAdrNorge, XMLStrukturertAdresse strukturertAdresse) {
        ...
        DateTime gyldigFra = periode == null ? null : periode.getFom();
        DateTime gyldigTil = periode == null ? null : periode.getTom();

        if (strukturertAdresse instanceof XMLGateadresse) {
            return getMidlertidigPostadresseNorge(gyldigFra, gyldigTil, (XMLGateadresse) strukturertAdresse);
        } else if (strukturertAdresse instanceof XMLPostboksadresseNorsk) {
            return getMidlertidigPostboksadresseNorge(gyldigFra, gyldigTil, (XMLPostboksadresseNorsk) strukturertAdresse);
        } else if (strukturertAdresse instanceof XMLMatrikkeladresse) {
            return getMidlertidigOmrodeAdresse(gyldigFra, gyldigTil, (XMLMatrikkeladresse) strukturertAdresse);
        }
        throw new SendSoknadException("ukjent adressetype med klassenavn: " + strukturertAdresse.getClass().getCanonicalName());
    }
    ...
    private Adresse hentBostedsAdresse(XMLGateadresse xmlGateAdresse) {
        ...
        String stringBuilder = xmlGateAdresse.getGatenavn() + ' ' +
                getHusnummer(xmlGateAdresse) +
                getHusbokstav(xmlGateAdresse) + ", " +
                getPostnummerString(xmlGateAdresse) + ' ' +
                kodeverk.getPoststed(getPostnummerString(xmlGateAdresse));
        adresse.setAdresse(stringBuilder);
